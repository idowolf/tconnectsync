...
class TandemSourceAutoupdate:
    ...
    def process(self, tconnect, nightscout, pretend, features=None):
        ...
        while True:
            ...
            # A transient failure (network outage, API hiccup, expired
            # session) must not kill the loop: log it, back off, retry.
            try:
                ...
                logger.error("Autoupdate cycle failed: %s" % e, exc_info=True)
                if self.secret.AUTOUPDATE_RESTART_ON_FAILURE:
                    logger.error("Exiting with error code due to AUTOUPDATE_RESTART_ON_FAILURE")
                    return 1
                logger.info("Retrying in %d seconds" % int(self.secret.AUTOUPDATE_DEFAULT_SLEEP_SECONDS))
