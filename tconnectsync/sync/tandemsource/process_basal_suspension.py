...
class ProcessBasalSuspension:
    ...
    def process(self, events: Iterable, time_start: arrow.Arrow, time_end: arrow.Arrow) -> List[dict]:
        ...
        for event in sorted(events, key=lambda x: x.eventTimestamp):
            ...
            entry = self.suspension_to_entry(event)
            if entry:
    ...
    def suspension_to_entry(self, event: "BaseEvent") -> Optional[dict]:
        if UPLOAD_DESTINATION == 'tidepool':
            return self.suspension_to_tidepool(event)
        return self.suspension_to_nsentry(event)

    def suspension_to_tidepool(self, event: "BaseEvent") -> Optional[dict]:
